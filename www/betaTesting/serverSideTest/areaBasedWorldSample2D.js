...
function modifiedControls()
{
	for (var i = 0; i <= maxSprites; i++)
	{
		if (spriteUser[areaId][i] == true)
		{
			...
			if(isRightMouseDown)
			{
				tpRectSprite(202, 200, 100, 100, 0, 2)
			}
		}
...
function sampleArea1()
{
	...
	displayRectPlayers(0, [0, 100, 255, 1]);
	areaRoom(500, 500, 500, 500, 50, ["left","right","top"], [200,100,300], [125,150,110], [0, 255, 255, 1], 32, 0);
	areaRect(100, 100, 100, 100, [0, 255, 0, 1], true, false, 30, 0);
	areaRect(0, 0, 1500, 1000, [100, 0, 0, 1], false, false, 48, 0);
	drawArea(1500, 1000, 0);
	openWorldCamera(0);
	stopAreaUser(0);
	if(mouseOver(0, 0, canvas.width/20, canvas.height/20))
	{
		...
		if(isLeftMouseDown)
		{
			...
			window.location = windowDirectory.replace("serverSideTest/", "");
		}
	}
